...
def test_set_price_feed_contract():
    ...
    # Assert
    assert (
        token_farm.tokenPriceFeedMapping(dapp_token.address) == price_feed_address
    ...
    ):  # we also have to  make sure that some body else should not call the get account function
        token_farm.setPriceFeedContract(
            dapp_token.address, price_feed_address, {"from": non_owner}
        )


# we need to write to test for staking tokens in order to check issue tokens
...
def test_issue_tokens(amount_staked):
    ...
    # Act
    token_farm.issueTokens({"form": account})
    # Arrange
